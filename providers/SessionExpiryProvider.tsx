...
import { AppState, Image, Modal, Platform, Pressable, StyleSheet, Text, View } from 'react-native';
...
export default function SessionExpiryProvider({ children, thresholdSec = 10, logo }: Props) {
    ...
    // Control refs
    const shownRef = useRef(false);        // evita múltiples prompts simultáneos
    const closingRef = useRef(false);      // evita dobles signOut
    const suppressRef = useRef(false);     // bloquea re-abrir tras “Cerrar”
    const intervalRef = useRef<number | null>(null);
    const currentExpRef = useRef<number | null>(null);

    // Anim
    const open = useSharedValue(0);
    const modalA = useAnimatedStyle(() => ({
        opacity: open.value,
        transform: [{ translateY: withTiming(open.value ? 0 : 16, { duration: 180 }) }],
    }));
    ...
    useEffect(() => {
        ...
        const sub = AppState.addEventListener('change', async (state) => {
            ...
            shownRef.current = false; // evita quedarse “pegado” al volver
            if (!suppressRef.current) {
                await startWatcher();
            } else {
                // Si el usuario cerró sesión manualmente, no reabrir prompt;
                // de todas formas, si el token ya no está, AuthGate te llevará a login.
            }
        });
        return () => sub.remove();
    }, [startWatcher]);
